...
namespace Final
{
    public partial class frmDormitoryOwner : Form
    {
        public frmDormitoryOwner()
        {
            InitializeComponent();
        }
        public long UserID;
        DormitoryDbContext db = new DormitoryDbContext();
        private void frmDormitoryOwner_Load(object sender, EventArgs e)
        {
        ...
        private void RefreshUsersList(List<Models.User> Userslist)
        {
            ...
            {
                //فقط نمایش مسئولان خوابگاه
                if (Role.FindRole(item.Id) == (int)EnumTool.Role.DormitoryOwner)
